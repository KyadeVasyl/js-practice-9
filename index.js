...
function findElementGreaterThan(arr, num) {
  ...
  const foundElement = arr.find((elem) => {
    if (elem > num) {
      return elem;
    }
  })
  ...
  // Якщо ні повертаємо рядок Числа яке задовільняє умову не знайдено
}
...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = arr.findIndex((elem) => name === elem.name);
  return foundIndex;
...
function getPositiveArrayValues(arr) {
  ...
  const getObjIterator = arr.values();
  // Конвертуємо ітератор в масив.
  const positiveArrayValues = Array.from(getObjIterator).filter((elem) => elem > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const removed = arr.splice(startIdx);
  // За допомогою методу map() подвоюємо кожен елемент масиву.
  const doubledArray = removed.map((elem) => elem * 2);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squeared = flatten.map((elem) => elem ** 2);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  const filteredSquaredArray = squeared.filter((elem) => elem > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = sliced.reduceRight((accumulator, currentValue) => accumulator + currentValue, 0);
  // За допомогою методу forEach() виводимо кожен елемент з останніх n в консоль.
  sliced.forEach((elem) => console.log(elem));
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((longest, currentWord) => {
    if (currentWord.length > longest.length) {
      return currentWord; // Якщо поточне слово довше, повертаємо його
    } else {
